...
export default class Chart extends Component {
  ...
  createNewWeeksArray(array) {
    ...
    if (array === undefined) return newArray;
  ...
  render() {
    ...
    if (months.min === '300010') return null;
      ...
      <div>
        ...
        <div className="flex-container flex-center">
          ...
          {this.props.data.headers.map((header, index) => {
            ...
            if (ranks.peaks[index] === 101) return null;

            return (
              <div key={header} className="Chart-legend text-center">
                <div style={style}>
                  <b>{this.getAbbr(header)}</b>
                </div>
  ...
  getX(week, months) {
    ...
    return (
      ((date - months.start) / (months.end - months.start)) *
        (size - inset * 3) +
      inset * 2
    );
  }
  ...
  getXAxis(months) {
    ...
    if (i <= 12) {
      for (j = 0; j < i; j++) {
        date = new Date(Date.UTC(startYear, startMonth + j - 1, 1))
          .toISOString()
          .split('-');
        year = date[0];
        month = date[1];
        x = xs[j];
        markers.push(
          <text key={j} x={(prev.m + x) / 2} y={y2 + 8} style={markerStyle}>
            {month}
          </text>
        );
        prev.m = x;
    ...
    weeks.forEach(week => {
      ...
      week.ranks.forEach((rank, index) => {
        if (rank !== '-' && rank <= 100) {
          ...
          if (ranks.peaks[index] > rank) ranks.runs[index] = 0;
          ranks.peaks[index] = Math.min(ranks.peaks[index], rank);
          ranks.wocs[index]++;
          if (ranks.peaks[index] === rank) ranks.runs[index]++;
          valid = true;
        }
      });

      if (valid) {
        if (months.min > month) months.min = month;

        if (months.max < month) months.max = month;
      }
    });

    months.start = new Date(
      Date.UTC(months.min.substring(0, 4), months.min.substring(4, 6) - 1, 1)
    );
    months.end = new Date(
      Date.UTC(months.max.substring(0, 4), months.max.substring(4, 6), 0)
    );
