...
export default class CurrentList extends Component {
  ...
  render() {
    ...
    const filtered =
      this.state.filtered.length > 0 ? this.state.filtered : songs;
    ...
    return (
      <div>
        <div style={gridStyle}>
          {this.getHeaderView()}
          <div style={spacerStyle} className="text-center">
            {filtered.length}
          </div>
        </div>
              ...
              <div className="overflow-hidden">
                ...
                <div className="ellipsis">
                  ...
                  {song.features.length > 0 && (
                    <span>
                      {' '}
                      feat. <NameArray array={song.features} />
                    </span>
                  )}
                </div>
              </div>
  ...
  filterBy(chart) {
    ...
    songs.forEach(song => {
      if (song[chart]) filtered.push(song);
    });
  ...
  filterByPlayCount() {
    ...
    filtered.sort((a, b) =>
      a.plays !== b.plays ? b.plays - a.plays : a.index - b.index
    );
  ...
  getRankStyle(rank) {
    ...
    if (rank <= 1) style.backgroundColor = 'rgb(255, 45, 85)';
    else if (rank <= 5) style.backgroundColor = 'rgb(88, 86, 214)';
    else if (rank <= 10) style.backgroundColor = 'rgb(0, 122, 255)';
    else style.backgroundColor = 'rgba(255, 255, 255, 0.2)';
    ...
    return (
      <div style={gridStyle} className="text-center">
        {charts.map(chart => {
          var rank = song[chart];
          return (
            <div key={chart} style={this.getRankStyle(rank)}>
              {rank ? rank : <span>&nbsp;</span>}
            </div>
          );
        })}
      ...
      <div style={gridStyle} className="text-center">
        {charts.map(chart => {
          return (
            <div
              key={chart}
              style={this.getRankStyle(100)}
              onClick={() => this.filterBy(chart)}
            >
              {abbrs[chart]}
            </div>
          );
        })}
        <div className="lightgray" onClick={() => this.filterByPlayCount()}>
          P
        </div>
      </div>
    );
