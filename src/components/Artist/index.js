...
export default class Artist extends Component {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="top text-center">{TextUtil.normalize(artist.name)}</div>
        <div className="text-center">{this.getDescription()}</div>
  ...
  createSingle(song) {
    ...
    for (i in song.rank) {
      if (song.rank[i]) {
        if (min > song.rank[i].min) count = 0;

        min = Math.min(min, song.rank[i].min);
        run += song.rank[i].run;

        if (min === song.rank[i].min) count += song.rank[i].count;
      }
    }
      ...
      artist.singleGroups.forEach(group => {
        if (singles[group[0]]) {
          ...
          group.forEach((songId, index) => {
            ...
            if (index > 0) singles[songId].merged = true;
          });
          single.songs = songs;
        }
      });
  ...
  cmpSingle(a, b) {
    if (a.rank.min !== b.rank.min) return a.rank.min - b.rank.min;

    if (a.rank.count !== b.rank.count) return b.rank.count - a.rank.count;

    if (a.rank.run !== b.rank.run) return b.rank.run - a.rank.run;
  ...
  cmpSong(a, b) {
    ...
    if (a.album.release === b.album.release) return a.album.id - b.album.id;
  ...
  getFeatures(albums) {
    ...
    albums.forEach(album => {
      album.songs.forEach(song => {
        if (song.album === album) songs[song.id] = song;
      });
    });
  ...
  getDirectAlbums(artist) {
    ...
    artist.albums.sort((a, b) => {
      if (a.release === b.release) return a.id - b.id;
  ...
  getDescription() {
    ...
    if (artist.gender !== null) desc += artist.gender + ' ';

    if (artist.type !== null) desc += artist.type + ' ';

    if (artist.origin !== null) desc += 'from ' + artist.origin;
  ...
  getViews(artist) {
    ...
    if (this.cmpIds(songs.map(s => s.id), features)) features = [];
    ...
    if (singles.length > 0)
      views.push({
        ...
        data: {
          singles: singles,
          id: artist.id,
          peaks: this.formatSinglePeaks(artist)
        }
      });
    if (songs.length > 0)
      views.push({
