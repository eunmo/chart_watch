...
for my $span ($dom->find('span[class="chart-element__information"]')->each) {
  $title = normalize($span->find('span[class*="chart-element__information__song"]')->first->all_text);
  $artist = normalize($span->find('span[class*="chart-element__information__artist"]')->first->all_text);

  print ",\n" if $rank > 1;
  print "{ \"rank\": $rank, \"artist\": \"$artist\", \"titles\" : [\"$title\"] }";
  $rank++;
}
