...
module.exports = {
  matchWeek: function(models, db, chartName, date) {
    function findArtistByName(name, chart) {
      return db.promisifyQuery(
        'Select id from Artists ' +
          ...
          name +
          '" or ' +
          'id = (select ArtistId from ArtistAliases where alias = "' +
          name +
          '" and chart = "' +
          chart +
          '")'
    ...
    function findArtist(entry) {
      ...
      return findArtistByName(artist, chart).then(function(results) {
        if (results.length > 0) {
          ...
          if (artist !== nameNorm) {
            return findArtistByName(nameNorm, chart).then(function(results) {
              if (results.length > 0) {
                entry.artistId = results[0].id;
              } else {
                ...
                if (artist !== nameInv && nameNorm !== nameInv) {
                  return findArtistByName(nameInv, chart).then(function(
                    results
                  ) {
                    if (results.length > 0) {
                      entry.artistId = results[0].id;
    ...
    var findSongByArtist = function(artistId, title, chart) {
      var query =
        ...
        '(select SongId from SongArtists where ArtistId = ' +
        artistId +
        ') ' +
        'and (title = "' +
        title +
      ...
      return db
        .promisifyQuery(
          'Select id, title from Songs where id in ' +
            ...
            '	(select AlbumId from AlbumArtists where ArtistId = ' +
            artistId +
            ')) ' +
            'and (title = "' +
            title +
    ...
    function findSongByFunction(
      ...
      return queryFunction(artistId, title, chart).then(function(results) {
    ...
    function handleTitleException(artistId, title, chart, date) {
      if (artistId === 52) {
        ...
        if (title === 'Finesse' || title === 'FINESSE') {
          if (date >= FinesseRemixDate) {
            return 'Finesse (Remix)';
          }
        }
      } else if (artistId === 287) {
        // Ed Sheeran
      ...
      } else if (artistId === 333) {
        // 소녀시대
        if (chart === 'oricon') {
          if (title === 'Oh!' || title === 'Run Devil Run' || title === 'Gee') {
            return title + ' (Japanese Ver.)';
          }
        }
      } else if (artistId === 681) {
        // 방탄소년단
        if (chart === 'oricon') {
          if (title === 'MIC Drop' || title === 'DNA') {
            return title + ' (Japanese Ver.)';
          }
        }
      } else if (artistId === 704) {
        // 동방신기
        if (chart === 'melon') {
