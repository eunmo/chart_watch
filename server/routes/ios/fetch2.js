(function () {
	...
	module.exports = function (router, _, db) {
		...
		function getAlbumSongs(albumMap, songMap) {
			...
			return db.album.getSongs(albumIds)
		...
		function getSongArtists(songMap, artistMap) {
			...
			return db.song.getArtists(songIds)
			.then(function (songArtists) {
				for (songId in songArtists) {
					array = songMap[songId].artists = toIdArray(songArtists[songId].artists);
					addArrayToMap(array, artistMap);
					array = songMap[songId].features = toIdArray(songArtists[songId].features);
					addArrayToMap(array, artistMap);
				}
			});
		}
		...
		function getAlbumArtists(albumMap, artistMap) {
			...
			return db.album.getArtists(albumIds)
