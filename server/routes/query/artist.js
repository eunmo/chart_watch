(function() {
  ...
  var songCmp = function(a, b) {
    if (a.disk === b.disk) return a.track - b.track;
    return a.disk - b.disk;
  };

  var findArtist = function(db, artist) {
    return db.artist.getRow(artist.id).then(function(rows) {
      if (rows.length > 0) {
        var row = rows[0];
  ...
  var findAs = function(db, artist) {
    return db.artist.getA(artist.id).then(function(rows) {
      artist.As = rows;
  ...
  var findBs = function(db, artist) {
    return db.artist.getBs([artist.id]).then(function(Bs) {
      artist.Bs = Bs[artist.id];
    });
  };

  var findAlbumsAndSongs = function(db, artist, maps) {
    return db.artist.getAlbumsAndSongs(artist.ids).then(function(rows) {
      ...
      for (i in albumMap) {
        ...
        for (j = 1; j < album.songs.length; j++) {
          a = album.songs[j - 1];
          b = album.songs[j];
          if (a.disk !== b.disk || a.track !== b.track) songs.push(b);
        }
        album.songs = songs;
        albums.push(album);
      ...
      for (i in albums) {
        for (j in albums[i].songs) {
          if (songMap[albums[i].songs[j].id] === undefined)
            songMap[albums[i].songs[j].id] = {
              id: albums[i].songs[j].id,
              songs: []
            };
  ...
  var findAlbumDetails = function(db, artist, maps) {
    db.album.getDetails(artist.albumIds).then(function(rows) {
      var i, album, row;
  ...
  var findAlbumArtists = function(db, artist, maps) {
    return db.album.getArtists(artist.albumIds).then(function(albumArtists) {
  ...
  var findAlbumChartSummary = function(db, artist, maps) {
    return db.chartSummary.getAlbums(artist.albumIds).then(function(charts) {
      for (var i in charts) {
  ...
  var findSongDetails = function(db, artist, maps) {
    return db.song.getDetails(artist.songIds).then(function(rows) {
  ...
  var findSongArtists = function(db, artist, maps) {
    return db.song.getArtists(artist.songIds).then(function(songArtists) {
      var i, j;
  ...
  var findSongChartSummary = function(db, artist, maps) {
    return db.chartSummary.getSongs(artist.songIds).then(function(charts) {
      var i, j, songs;
      for (i in charts) {
  ...
  var groupSingles = function(db, artist) {
    ...
    var query =
      ...
      ' FROM SingleCharts WHERE SongId in (' +
      ids.join(',') +
      ')' +
