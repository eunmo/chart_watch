(function() {
  ...
  var getDetail = function(db, album) {
    return db.album.getDetails([album.id]).then(function(rows) {
      var row = rows[0];
  ...
  var getArtist = function(db, album) {
    return db.album.getArtists([album.id]).then(function(albumArtists) {
      album.artists = albumArtists[album.id];
    });
  };

  var getChart = function(db, album) {
    return db.chartWeeks.getOneAlbum(album.id).then(function(charts) {
      album.charts = charts;
    });
  };
  ...
  var getSongs = function(db, album) {
    return db.album.getSongs([album.id]).then(function(albumSongs) {
      album.songs = albumSongs[album.id];

      var songs = album.songs;
      var songIds = songs.map(song => {
        return song.id;
      });
      var promises = [
  ...
  module.exports = function(router, _, db) {
    ...
    router.get('/api/album/format/:_format', function(req, res) {
      ...
      return db.album
        .getByFormat(req.params._format)
        .then(function(rows) {
          var i, row;
    ...
    router.get('/api/album/format2/:_format', function(req, res) {
      ...
      return db.album
        .getByFormat2(req.params._format)
        .then(function(rows) {
          var i, row;
    ...
    router.get('/api/all-albums', function(req, res) {
      ...
      db.promisifyQuery(query)
        .then(function(rows) {
          ...
          query =
            'Select AlbumId, ArtistId, `order`, name FROM AlbumArtists a, Artists b where a.ArtistId = b.id';
        ...
        .then(function(rows) {
          for (var i in rows) {
            row = rows[i];
            albums[row.AlbumId].artists[row.order] = {
              id: row.ArtistId,
              name: row.name
            };
          }
