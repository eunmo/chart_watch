(function() {
  ...
  module.exports = function(router, _, db) {
    ...
    router.get('/chart/single/view/:_chart', function(req, res) {
      ...
      return getWeek(chartName, date).then(function(songs) {
        var ids = [];

        for (var i in songs) {
          if (songs[i].id !== null) ids.push(songs[i].id);
        }
    ...
    router.get('/api/chart/single/view/full/:_chart/:_week', function(
      req,
      res
    ) {
      var chartName = req.params._chart;
      ...
      Promise.all([
        ...
        thisWeekRows.forEach(row => {
          if (row.id != null) {
            ...
            nonMatches[row.artist + row.title + row.order] === undefined
          )
        ...
        lastWeekRows.forEach(row => {
          ...
          if (id !== null && songs[id] !== undefined) {
            if (songs[id].lastWeek === undefined) songs[id].lastWeek = row.rank;
            else songs[id].lastWeek = Math.min(songs[id].lastWeek, row.rank);
          }
        ...
        promises.push(db.song.fetchOldestAlbum(songs, ids));
        promises.push(
          db.chartSummary.fetchSongsByType(songs, ids, chartName, thisWeek)
        );
