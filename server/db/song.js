(function() {
  ...
  module.exports = function(db) {
    ...
    db.song.get = function(columns, filter) {
      var query =
        'SELECT id, title, plays ' + columns + '  FROM Songs ' + filter + ';';
    ...
    db.song.getAlbums = function(ids) {
      ...
      return db.promisifyQuery(query).then(function(rows) {
        ...
        for (i in songAlbums) {
          songAlbums[i].sort(function(a, b) {
    ...
    db.song.fetchDetails = function(songs, ids) {
      return db.song.getDetails(ids).then(function(rows) {
        var i, row, song;
        var details = {};
    ...
    db.song.fetchArtists = function(songs, ids) {
      return db.song.getArtists(ids).then(function(songArtists) {
        var i, song;
    ...
    db.song.fetchOldestAlbum = function(songs, ids) {
      return db.song.getAlbums(ids).then(function(songAlbums) {
        var i, song;
    ...
    db.song.fetchChartSummary = function(songs, ids) {
      return db.chartSummary.getSongs(ids).then(function(charts) {
        var i, song;
        for (i in songs) {
          song = songs[i];
