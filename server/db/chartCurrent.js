  ...
  module.exports = function(db) {
    ...
    function getCurrentSongIds(currentWeeks) {
      ...
      for (var i = 0; i < currentWeeks.length; i++) {
        ...
        query +=
          ...
          "' " +
          "   AND `week` = '" +
          currentWeeks[i].week.toISOString() +
          "' " +
    ...
    function sortCurRank(a, b) {
      return a.rank === b.rank
        ? charts.indexOf(a.type) - charts.indexOf(b.type)
        : a.rank - b.rank;
    }
    ...
    function sortSong(a, b) {
      ...
      if (a.curRank.length === b.curRank.length) {
        ...
        return (
          charts.indexOf(a.curRank[0].type) - charts.indexOf(b.curRank[0].type)
        );
      }
    ...
    function sortCurrentSongIds(currentSongIds) {
      ...
      for (i in currentSongIds) {
        ...
        songMap[row.SongId].curRank.push({
          type: row.type,
          rank: row.rank,
          order: row.order
        });
    ...
    function getCount(weeks, chart, column) {
      ...
      return db.promisifyQuery(query + ';').then(function(counts) {
        var week, count;
        var i, j;
    ...
    db.chartCurrent.getSingleSummary = function() {
      return getCurrentWeeks('SingleCharts').then(function(weeks) {
        var promises = [];
    ...
    db.chartCurrent.getAlbumSummary = function() {
      ...
      return getCurrentWeeks('AlbumCharts').then(function(weeks) {
        var promises = [];
