...
musicApp.controller('ArtistInitialCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('ArtistCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  var hideRedundantArtists = function(artist) {
    ...
    for (i in artist.albums) {
      ...
      for (j in album.songs) {
        ...
        if (album.albumArtists.length !== song.artists.length) continue;
  ...
  var getA = function(artist) {
    ...
    for (i in artist.As) {
      A = artist.As[i];
      if (As[A.type] === undefined) As[A.type] = [];
  ...
  $http.get('api/artist/' + $routeParams.id).success(function(artist) {
    ...
    for (i in artist.albums) {
      ...
      album.disks = [];
      for (j in disks) {
        disks[j].songs.sort(function(a, b) {
          return a.track - b.track;
        });
        album.disks.push(disks[j]);
      }
      album.disks.sort(function(a, b) {
        return a.disk - b.disk;
      });
      if (
        album.disks.length >= 1 &&
        album.disks[album.disks.length - 1].disk > 1
      ) {
        album.showDisk = true;
      }
      ...
      else years[year].albums.push(album);
    }
    ...
    if (artist.albums.length === 1) $scope.selectAlbum(artist.albums[0]);
  ...
  var getSendSong = function(song, album) {
    ...
    if (minRank < 10) sendSong.rank = minRank;
...
musicApp.controller('AddArtistAlbumCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
...
musicApp.controller('EditArtistCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $scope.addRelation = function() {
    if (
      $scope.artist.editRelations.length === 0 ||
      $scope.artist.editRelations[$scope.artist.editRelations.length - 1]
        .name !== null
    ) {
      var artist = {
        name: null,
  ...
  $scope.addAlias = function() {
    if (
      $scope.artist.editAliases.length === 0 ||
      ($scope.artist.editAliases[$scope.artist.editAliases.length - 1].alias !==
        null &&
        $scope.artist.editAliases[$scope.artist.editAliases.length - 1]
          .chart !== null)
    ) {
      var alias = {
        alias: null,
...
musicApp.controller('EditAlbumCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $scope.addAlias = function() {
    if (
      $scope.album.editAliases.length === 0 ||
      ($scope.album.editAliases[$scope.album.editAliases.length - 1].alias !==
        null &&
        $scope.album.editAliases[$scope.album.editAliases.length - 1].chart !==
          null)
    ) {
      var alias = {
        alias: null,
...
musicApp.controller('EditSongCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $scope.addAlias = function() {
    if (
      $scope.song.editAliases.length === 0 ||
      ($scope.song.editAliases[$scope.song.editAliases.length - 1].alias !==
        null &&
        $scope.song.editAliases[$scope.song.editAliases.length - 1].chart !==
          null)
    ) {
      var alias = {
        alias: null,
...
musicApp.controller('MetaChartCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $http.get('chart/single/' + $routeParams.type).success(function(chartRows) {
  ...
  $scope.getRankCellStyle = function(rank) {
    ...
    return (
      'color: white; text-align: center; font-weight: bold; background-color: ' +
      (rank < 6 ? colors[5 - rank] : '#c6dbef')
    );
  };
});

musicApp.controller('AlbumChartCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $scope.view = function() {
    $scope.rows = [];
    $http
      .get('chart/album/view/' + $scope.chart, {
        params: {
  ...
  $scope.fetch = function() {
    $http
      .get('chart/album/fetch/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
  ...
  $scope.match = function() {
    $http
      .get('chart/album/match/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
  ...
  $scope.clear = function() {
    $http
      .get('chart/album/clear/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
...
musicApp.controller('SingleChartCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $scope.view = function() {
    $scope.rows = [];
    $http
      .get('chart/single/view/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
  ...
  $scope.fetch = function() {
    $http
      .get('chart/single/fetch/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
  ...
  $scope.match = function() {
    $http
      .get('chart/single/match/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
  ...
  $scope.clear = function() {
    $http
      .get('chart/single/clear/' + $scope.chart, {
        params: {
          year: $scope.date.getFullYear(),
          month: $scope.date.getMonth() + 1,
          day: $scope.date.getDate()
...
musicApp.controller('SingleOnesCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $http
    .get('chart/single/ones/' + $routeParams.name)
...
musicApp.controller('OneSongsCtrl', function(
  $rootScope,
  $scope,
  $http,
  songService
  ...
  $http.get('chart/ones').success(function(results) {
...
musicApp.controller('ChartHistoryCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $http.get('chart/history/' + $routeParams.name).success(function(results) {
    $scope.headers = results.headers;
    $scope.weeks = results.weeks;
  });
...
musicApp.controller('RecentCtrl', function(
  $rootScope,
  $scope,
  $http,
  songService
  ...
  $scope.refresh = function() {
    $http.get('api/lastPlayed').success(function(lastPlayed) {
...
musicApp.controller('NewSongCtrl', function($rootScope, $scope, $http) {
  ...
  $scope.refresh = function() {
    $http.get('api/newSongs').success(function(lastPlayed) {
      $scope.rows = lastPlayed;
    });
  };
  $scope.refresh();
...
musicApp.controller('ChartMissingAlbumCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('ChartMissingAlbumYearCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
  ...
  $http
    .get('chart/missing/album/' + $routeParams.rank + '/' + $routeParams.year)
    .success(function(data) {
      $scope.data = data;
    });
});

musicApp.controller('AlbumCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('SongCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('BatchCtrl', function($rootScope, $scope, $http) {
      ...
      if (chart.cur.getTime() > chart.end.getTime()) {
        ...
        chart.progress =
          ((chart.cur.getTime() - chart.start.getTime()) /
            (chart.end.getTime() - chart.start.getTime())) *
          100;
        break;
      }
    }
  ...
  $scope.run = function() {
    ...
    if ($scope.chartIndex < $scope.charts.length) {
      $http
        .get($scope.urlPrefix + chart.chart, {
          params: {
...
musicApp.controller('AlbumFormatCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
  ...
  $http
    .get('api/album/format/' + $routeParams.format)
...
musicApp.controller('AlbumFormat2Ctrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
  ...
  $http
    .get('api/album/format2/' + $routeParams.format)
    .success(function(albums) {
      $scope.albums = albums;
      $scope.loaded = true;
    });
...
musicApp.controller('AddAlbumChartNoteCtrl', function(
  $rootScope,
  $scope,
  $http
) {
...
musicApp.controller('IOSCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('SeasonSingleCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
...
musicApp.controller('SeasonSingleListCtrl', function(
  $scope,
  $http,
  songService
) {
...
musicApp.controller('StatsPlaysCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http
  ...
  $http.get('stats/plays-by-' + $routeParams.type).success(function(data) {
    ...
    for (i = 0; i <= 100; i++) {
      ...
      for (j = 1; j <= 10; j++) {
        $scope.plays[i].cumul[j] =
          $scope.plays[i].cumul[j - 1] + $scope.plays[i].rank[j];
      }
    }
  ...
  function updateData() {
    ...
    for (i in $scope.ranks) {
      if ($scope.ranks[i].active) data = data.concat($scope.ranks[i].data);
      else $scope.allVisible = false;
    }
...
musicApp.controller('SummarySingleChartCtrl', function(
  $rootScope,
  $scope,
  $http
) {
...
musicApp.controller('SummaryAlbumChartCtrl', function(
  $rootScope,
  $scope,
  $http
) {
...
musicApp.controller('SingleChartAlbumCtrl', function(
  $rootScope,
  $scope,
  $routeParams,
  $http,
  ...
  $http
    .get('chart/single/album/' + $scope.chart, {
      params: {
        year: $scope.date.getFullYear(),
        month: $scope.date.getMonth() + 1,
        day: $scope.date.getDate()
