      ...
      return {
        pre: function(scope, element, attrs) {
          ...
          if (scope.s !== undefined) {
            ...
            scope.inner =
              'max-height: ' + scope.s + 'px; max-width: ' + scope.s + 'px';
      ...
      return {
        pre: function(scope, element, attrs) {
          ...
          if (scope.rank) {
            ...
            if (scope.rank.run) scope.run = scope.rank.run;
            if (scope.min <= 10) scope.show = true;
            if (scope.count > 1) scope.showCount = true;
  ...
  return {
    ...
    link: function(scope, element) {
      ...
      scope.render = function(data) {
        ...
        var i;
        var maxCount = d3.max(dataSeries, function(d) {
          return d.count + d.offset;
        });

        var xMax =
          Math.ceil(
            d3.max(dataSeries, function(d) {
              return d.plays;
            }) / 10
          ) * 10;
        ...
        if (maxCount < 200) {
          ...
          for (i = 10; i <= yMax; i += 10) yTicks.push(i);
          ...
          // go polylinear if max count > 100
          var mill = Math.ceil(maxCount / 1000);
          var unit = mill * 100;
          yMax = Math.ceil(maxCount / unit) * unit;

          for (i = 10; i < 100 && i < yMax; i += 10) yTicks.push(i);
          for (i = 100; i <= yMax; i += unit) yTicks.push(i);
        ...
        bars
          .attr('x', function(d) {
            return x(d.plays) - barWidth / 2;
          })
          .attr('width', barWidth)
          .attr('height', function(d) {
            return y(d.offset) - y(d.count + d.offset + 0);
          })
          .attr('y', function(d) {
            return y(d.count + d.offset + 0);
          })
          .style('fill', function(d) {
            return color(d.rank);
          })
          .on('click', function(d, i) {
            scope.showSongs({ play: d.plays });
          })
          .on('mouseover', function(d) {
            var text =
              '<table class="table table-bordered table-tooltip" id="d3-bar-plays-tooltip">' +
              '<tr><td>' +
              d.total +
              '</td></tr>' +
              '</table>';
            tooltip.html(text).style('opacity', 1.0);
        ...
        // tooltip
        tooltip = svg.append('foreignObject').attr('class', 'tooltip');
      };
